...
/**
 * Map many ISINs via OpenFIGI `/v3/mapping` (up to 10 ids per HTTP request).
 */
export async function fetchOpenFigiMappingsByIsins(
  isins: string[],
  options?: { beforeEachChunk?: () => Promise<void> },
): Promise<Map<string, OpenFigiMappingRow[]>> {
  const out = new Map<string, OpenFigiMappingRow[]>();
  const unique = [
    ...new Set(
  ...
  const beforeChunk = options?.beforeEachChunk ?? (async () => {});
