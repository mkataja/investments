...
| [`web`](web) | Vite + React + Tailwind; portfolio UI, **brokers** at `/brokers`, **instruments list** at `/instruments`, **new instrument** at `/instruments/new`, **Degiro CSV import** at `/import`, dev data checks |
...
- **`brokers`:** **`name`** is unique. **`broker_type`** is **`exchange`** (Yahoo-backed equities), **`seligson`** (custom integrations / mutual funds), or **`cash_account`**. CRUD via **`GET/POST/PATCH/DELETE /brokers`** ( **`/brokers`** in **`web`** ). Which instrument kinds are allowed per broker: **`isInstrumentKindAllowedForBrokerType`** in **`@investments/db`**.
...
- **`instruments`:** **`kind`** (`etf` | `stock` | `custom` | `cash_account`). **`broker_id`** is **null** for **`etf`/`stock`**, **required** for **`custom`** and **`cash_account`** (identifies which broker’s integration applies). **`custom`** rows may reference **`seligson_fund_id`** when that broker’s pipeline uses Seligson fund data. Optional **`mark_price_eur`** when Yahoo quotes are not used.
- **`transactions`:** **`trade_date`** is **`timestamptz`** (full instant). Trades with **`currency`** and optional **`unit_price_eur`** for EUR-side reporting; optional **`external_source`** / **`external_id`** for idempotent broker imports (e.g. Degiro **Order ID** UUID).
...
- **On create:** **`POST /instruments`** for ETF/stock (Yahoo) and **`custom`** (Seligson) **writes `distribution_cache` immediately** (same fetch path as refresh). Create fails with **502** if the cache write fails (instrument row is not left behind).
...
- **`GET /instruments`** returns each instrument row plus **`netQuantity`** (sum of buys minus sells), optional joined **`distribution`** (`fetchedAt`, `source`, `payload` with regions/sectors), optional **`broker`** summary (`id`, `name`, `brokerType`) when **`broker_id`** is set, and optional **`seligsonFund`** (`id`, `fid`, `name`) for **`custom`** Seligson-linked rows. Optional query **`brokerId`**: returns only instruments allowed for that broker’s **`broker_type`** (same rules as **`isInstrumentKindAllowedForBrokerType`**); **`400`** / **`404`** for invalid or missing broker id.
- **`GET /instruments/:id`** returns the same shape as one list element; **`404`** if missing.
- **`PATCH /instruments/:id`** updates an instrument (at least one field required; **`400`** if the body includes fields not allowed for that **`kind`**). **`etf`/`stock`**: display name, Yahoo symbol, ISIN, **`mark_price_eur`**; changing Yahoo symbol refetches Yahoo and rewrites the distribution cache. **`custom`**: display name, broker (Seligson-type), ISIN, **`mark_price_eur`**. **`cash_account`**: display name, broker (cash-account-type), **`cash_currency`**, **`cash_geo_key`**, **`cash_interest_type`**; duplicate cash display name (case-insensitive) returns **`409`**.
