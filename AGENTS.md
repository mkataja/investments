...
- **Cash (`cash_account`):** no external fetch for valuation beyond FX—nominal balance is in **`cash_currency`** (see **`SUPPORTED_CASH_CURRENCY_CODES`** in `db`); **`cash_geo_key`** is optional (ISO-like region key for display on **`/instruments`** only). Portfolio region/sector charts still **exclude** cash (see below).
