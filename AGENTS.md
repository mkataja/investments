...
- **IBKR CSV:** **`POST /import/ibkr`** with multipart field **`file`** (UTF-8 CSV). Parser in **`api/src/import/ibkrTransactions.ts`** supports (a) **flat Activity** export (**`ClientAccountID`**, **`DateTime`**, **`ISIN`**, **`ExchTrade`**, **`TradePrice`**, **`CurrencyPrimary`**) and (b) legacy Statement **`Transaction History`** **`Data`** rows (**`Date`**, **`Buy`/`Sell`**, **`Price`**, **`Price Currency`**). **Forex** is skipped: **`IDEALFX`** and symbols like **`EUR.USD`**. Flat **`ExchTrade`** uses signed **Quantity** (negative ⇒ sell). **`external_id`**: legacy **sha256** of **`symbol|txn type|qty|price`**; flat **sha256** of **`isin|symbol|ExchTrade|qty|price|DateTime`**. Resolution in **`api/src/import/ibkrResolveInstruments.ts`**: **`instruments.isin`** when the CSV ISIN matches a unique instrument, else **`instruments.yahoo_symbol`** (exact candidates, space vs **`-`** / **`.`**, then **listing suffix** match so IBKR **`SPYI`** can match stored **`SPYI.DE`**). **No** OpenFIGI. **`unit_price_eur`**: same **USD→EUR stub** as portfolio valuation when not EUR. Broker **`IBKR`**. **`400`**: **`missingSymbols`** / **`ambiguousSymbols`** / optional **`ambiguousIsins`** / **`missingIsins`** (CSV ISIN not in DB and Yahoo path failed). Success shape matches Degiro. Web: **`/import`**.
