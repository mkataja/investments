...
- **`instruments`:** **`kind`** (`etf` | `stock` | `custom` | `cash_account`). **`broker_id`** is **null** for **`etf`/`stock`**, **required** for **`custom`** (Seligson integration) and **`cash_account`** (points at the bank-style broker). **`custom`** rows still reference **`seligson_fund_id`** when the integration is Seligson. Optional **`mark_price_eur`** when Yahoo quotes are not used.
