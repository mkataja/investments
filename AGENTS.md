...
- **`PATCH /instruments/:id`** updates an instrument (at least one field required; **`400`** if the body includes fields not allowed for that **`kind`**). **`etf`/`stock`**: display name, Yahoo symbol, ISIN, **`mark_price_eur`**; changing Yahoo symbol refetches Yahoo and rewrites the distribution cache. **`custom`**: display name, broker (Seligson-type), ISIN, **`mark_price_eur`**. **`cash_account`**: display name, broker (cash-account-type), **`cash_currency`**, country code (**`cash_geo_key`**), **`cash_interest_type`**; duplicate cash display name (case-insensitive) returns **`409`**.
