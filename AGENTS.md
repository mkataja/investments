...
- **`brokers`:** **`broker_type`** is **`exchange`** (Yahoo-backed equities), **`seligson`** (custom integrations / mutual funds), or **`cash_account`**. CRUD via **`GET/POST/PATCH/DELETE /brokers`** ( **`/brokers`** in **`web`** ); optional **`code`** defaults from **`name`**. Which instrument kinds are allowed per broker: **`isInstrumentKindAllowedForBrokerType`** in **`@investments/db`**.
