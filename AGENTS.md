...
- **Optional provider holdings (ETF/stock):** **`instruments.holdings_distribution_url`** may point to an **HTTPS** file on **`ishares.com`** (CSV export) or **`ssga.com`** (daily XLSX). The API infers format from the hostname (**`validateHoldingsDistributionUrl`** / **`resolveHoldingsProviderKind`** in **`@investments/db`**); unsupported hosts **400** on **`POST`/`PATCH`**. When set, **`distributions`** come from the parsed file (**`ishares_holdings_csv`** / **`ssga_holdings_xlsx`**), raw text/base64 is stored in **`provider_holdings_cache`**, and **`yahoo_finance_cache`** is cleared for that instrument; **`prices`** still use Yahoo **`quoteSummary`** when **`yahoo_symbol`** is set. When unset, distributions remain Yahoo-only (same as before this feature). **Cash / cash-equivalent** rows (e.g. **Asset Class** or **Sector** on iShares; **Asset Class**, **Security Type**, or **Sector Classification** on SSGA) are attributed to **`sectors.cash`** and **omitted** from **`countries`**; **`GET /portfolio/distributions`** scales merged **country/region** weights by **`1 − sectors.cash`** per instrument so ETF cash does not inflate geographic exposure.
