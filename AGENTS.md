...
- **`PATCH /instruments/:id`** updates **`cash_account`** instruments (display name, broker, **`cash_currency`**, country code **`cash_geo_key`**) or **`etf`/`stock`** ( **`holdings_distribution_url`** only — **`null`** clears); **`400`** for other kinds. Duplicate cash display name (case-insensitive) returns **`409`**.
