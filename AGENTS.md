...
- **`GET /portfolio/distributions`** returns aggregated **`regions`** / **`sectors`**, **`totalValueEur`**, and **`positions`**: each row includes **`quantity`** (net open units), **`unitPriceEur`** (EUR position value ÷ quantity when meaningful), **`weight`**, **`valueEur`**, **`valuationSource`**, plus **`instrumentId`** / **`displayName`** (see **`api/src/lib/portfolio.ts`**).
