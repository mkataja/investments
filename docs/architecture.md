...
- `transactions`: `user_id` → `users`; `portfolio_id` → `portfolios` (required, `ON DELETE CASCADE`). `trade_date` is `timestamptz`. `currency`; optional `external_source` / `external_id` for idempotent imports (e.g. Degiro Order ID UUID).
