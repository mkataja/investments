...
- `users`: `name` text, required — no column default; placeholder for auth. One seeded row `name = default`. App uses `USER_ID` from `@investments/lib` (`appUser.ts`) for all scoping.
- `portfolios`: per user; unique `(user_id, name)` (`portfolios_user_name_uidx`); `kind` `live` \| `benchmark` (default `live`). **Live:** `transactions` reference `portfolio_id`; positions from net qty; `emergency_fund_eur` drives emergency vs excess cash in `GET /portfolio/distributions` asset mix. **Benchmark:** no transactions; `benchmark_total_eur` scales synthetic position EUR; target weights in `portfolio_benchmark_weights` (`instrument_id`, `weight`, `sort_order`); distributions merge instrument payloads using synthetic EUR notionals proportional to normalized weights (`api/src/lib/benchmarkPortfolio.ts`). `GET/POST /portfolios`, `PATCH /portfolios/:id` for `USER_ID` (`PATCH` can change `kind` — cannot switch live→benchmark if the portfolio has transactions; benchmark→live deletes weight rows). `GET/PUT /portfolios/:id/benchmark-weights` for benchmark lines. Home uses selected portfolio + `GET /portfolio/distributions?portfolioId=` / `GET /transactions?portfolioId=`. Imports: optional multipart `portfolioId` (defaults to lowest-id **live** portfolio; benchmark portfolios rejected).
- `brokers`: `user_id` → `users` (required). Unique `(user_id, name)` (`brokers_user_name_uidx`); `POST /brokers` uses `USER_ID`. No default brokers at startup. `broker_type`: `exchange` (Yahoo equities), `seligson` (mutual funds), `cash_account` (cash only — no equities). CRUD `GET/POST/PATCH/DELETE /brokers`. Allowed instrument kinds: `isInstrumentKindAllowedForBrokerType` in `@investments/lib` (`cash_account` broker → `cash_account` instruments only). Imports resolve brokers named Degiro, IBKR, Seligson within `USER_ID`.
...
- `instrument_composite_constituents`: optional rows per parent `instrument_id` — weighted blend of `target_instrument_id` or a `pseudo_key` (`other_equities`, `other_long_government_bonds`, `other_long_corporate_bonds`, `other_short_government_bonds`, `other_short_corporate_bonds`, `other_ultrashort_bonds`, `cash`). When present, distribution refresh merges child `distributions` instead of scraping the parent. See [seligson.md](./data-sources/seligson.md) (Pharos-style table).
- `transactions`: `user_id` → `users`; `portfolio_id` → `portfolios` (required, `ON DELETE CASCADE`). `trade_date` is `timestamptz`. `currency`, optional `unit_price_eur`; optional `external_source` / `external_id` for idempotent imports (e.g. Degiro Order ID UUID).
- `distributions`: `payload` `{ countries, sectors }` — ISO 3166-1 alpha-2 country keys (uppercase), canonical sector ids (`lib/src/distribution/sectorIds.ts`). **History:** one row per `(instrument_id, snapshot_date)` (UTC calendar day). Yahoo `snapshot_date` follows `quoteSummary.price.regularMarketTime` when present, else fetch time. `source`: `yahoo`, `seligson_scrape`, `composite`, `manual`, provider imports, etc. Conflict rules when the same day is written again: `composite` rows are only updated by composite refresh; issuer scrapes (e.g. `seligson_scrape`) override other non-composite sources; within the same tier, latest `fetched_at` wins. **Current UI** uses the row with the latest `snapshot_date` per instrument. Raw: `yahoo_finance_cache` (quoteSummary-shaped JSON), `seligson_distribution_cache` (`holdings_html` = FundViewer view=10); composite parents clear FundViewer cache for that instrument. Refresh clears `yahoo_finance_cache` when writing Seligson scrape for that instrument.
- `prices`: one row per `(instrument_id, price_date)` with `price_type` `intraday` \| `close` (same calendar day: `close` is not replaced by `intraday`). Yahoo ETF/stock: from `quoteSummary.price` when present (`intraday` if `marketState` is `REGULAR`, else `close`); `price_date` from `regularMarketTime` when present (else fetch time), same as Yahoo `distributions.snapshot_date`; Seligson `custom`: NAV from `FundValues_FI.html` (`seligson_fund_value_cache`), dated by each row `Pvm`. Trades can seed an `intraday` row when no price exists for the trade date (`transaction_seed`). **Current valuation** uses the latest `price_date` per instrument (`valuePortfolioRowsEur`). As-of-date valuation for asset mix history prefetches price rows up to each sample date, picks the latest with `price_date <= as_of` (`pickLatestPriceRowAsOf`), then `valuePortfolioRowsFromPriceMap`. Non-EUR/USD FX is a temporary stub. Cash accounts: no `prices` row (nominal `cash_currency`).
...
`GET /portfolio/asset-mix-history?portfolioId=` (POC): weekly estimated **equities** vs **cash** EUR (non–`cash_account` vs `cash_account` positions), via `getPortfolioAssetMixHistory` (`valuePortfolioRowsFromPriceMap`); series stops when a held non-cash position has no price on or before that date.
